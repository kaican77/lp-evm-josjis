...
/**
 * Buy `tokenAddr` with WETH using the BEST execution available. Routes through Uniswap-native
 * routing (v4 UniversalRouter for native pairs, Router02 across all v3 fee tiers) — never an
 * external aggregator.
 *
 * Buying on the exact fee tier you're about to farm (often a thin, high-fee pool) bleeds
 * price impact; best-route execution keeps the token side's value ≈ the WETH spent, so the
 * position fills fully. Spends EXACTLY `wethRaw` (wrap + Router02), so the caller's WETH-left
 * math holds.
 */
export async function swapWethToTokenBest(tokenAddr: string, wethRaw: bigint, feeHint?: number): Promise<SwapResult> {
  ...
  // v4 native path (UniversalRouter) — the deepest single pool wins; fallback Router02 (v3)
  try {
    const sw = await uniswapSwap(C.weth, ethers.getAddress(tokenAddr), wethRaw);
    if (sw && sw.amountOut > 0n) {
      log.info(`beli token via Uniswap (best route) → ${sw.amountOut}`);
      return { tx: sw.tx, amountOut: sw.amountOut };
    }
    log.warn("Uniswap tak bisa route WETH→token → fallback pool v3 tier terdalam");
  } catch (e) {
    log.warn(`Uniswap gagal (${(e as Error).message.slice(0, 80)}) → fallback pool v3`);
  }
