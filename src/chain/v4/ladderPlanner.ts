...
export const FUNI_BOUNDARIES_5 = [100, 500, 1200, 2200, 3500, 5000] as const;
export const FUNI_WEIGHTS_5 = [800, 1200, 1800, 2500, 3700] as const;
// 3-slice compact mode: near / mid / deep. FUNI itself defines only 5 slices;
// this keeps the same outer depth while avoiding a paper-thin terminal slice.
export const FUNI_BOUNDARIES_3 = [100, 2200, 5000] as const;
export const FUNI_WEIGHTS_3 = [2500, 3500, 4000] as const;
...
export function planBidLadder(
  ...
  if (!Number.isSafeInteger(currentTick) || !Number.isSafeInteger(tickSpacing) || tickSpacing <= 0) {
    throw new Error("currentTick/tickSpacing invalid");
  }
  if (totalDepositWei <= 0n) throw new Error("totalDepositWei harus > 0");
  ...
  const boundaries = scaleBoundaries(
    legCount === 5 ? FUNI_BOUNDARIES_5 : FUNI_BOUNDARIES_3,
    maxDownsideBps,
  );
  const weights = legCount === 5 ? FUNI_WEIGHTS_5 : FUNI_WEIGHTS_3;
