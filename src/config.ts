...
const ConfigSchema = z.object({
  ...
  lp: LpSchema,
  gasPriceGwei: z.number().min(0).default(0),
  maxGasUsd: z.number().min(0).default(0),
  watch: WatchSchema,
...
export const env = {
  ...
  // Krystal Cloud API key (https://cloud.krystal.app) — optional PnL cross-check source. When set,
  // /list + /watch reconcile the on-chain deposit basis against Krystal's indexed providedAmounts.
  krystalKey: (process.env.RH_KRYSTAL_KEY || "").trim(),
};
